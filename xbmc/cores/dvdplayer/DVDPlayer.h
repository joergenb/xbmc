...
public:
  ...
  virtual bool CacheRealtimeStream(void) const;

  enum ECacheState
  { CACHESTATE_DONE = 0
  , CACHESTATE_FULL     // player is filling up the demux queue
  , CACHESTATE_RT      // player is waiting for some data in each buffer
  ...
  virtual bool IsCaching() const { return m_caching == CACHESTATE_FULL || m_caching == CACHESTATE_RT; }
