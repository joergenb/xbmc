...
int CVDPAU::Decode(AVCodecContext *avctx, AVFrame *pFrame)
{
  ...
  while (1)
  {
    ...
    {
      CLog::Log(LOGWARNING, "CVDPAU::Decode: timed out waiting for picture, messages: %d, free %d",
                 msgs, usedPics);
      break;
    }
  }
  ...
  if (usedPics > 0)
  {
    // in particular for live streams
    // without the delay the asynchronous processing might empty DVD queues
    if (m_startupdelay)
    {
      m_startupdelay = false;
      Sleep(200);
